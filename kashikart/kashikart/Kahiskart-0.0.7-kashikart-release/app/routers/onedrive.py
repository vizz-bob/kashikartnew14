...
async def process_onedrive_file(
    ...
    try:
        ...
        return ProcessFileResponse(
            ...
            file_tracker=FileTrackerResponse.from_orm(file_tracker)
        )
...
async def list_files(
    ...
    return [FileTrackerResponse.from_orm(f) for f in files]
...
async def get_file_info(
    ...
    return FileTrackerResponse.from_orm(file)
...
async def get_sheet_data(
    ...
    if request.file_id:
        ...
        if file:
            file_info = FileTrackerResponse.from_orm(file)
    
    return GetDataResponse(
        total_count=total_count,
        data=[SheetDataResponse.from_orm(d) for d in data],
        file_info=file_info
    )
...
async def get_sheet_mapping(
    ...
    return HeaderMappingResponse.from_orm(mapping)

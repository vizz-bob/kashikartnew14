...
class Source(Base):
    ...
    # Login (plain, for UI / Excel imports)
    login_required = Column(Boolean, default=False)
    password = Column(Text, nullable=True)  # plain-text copy for UI display / Excel imports

    # Status
    status = Column(Enum(SourceStatus), default=SourceStatus.ACTIVE, index=True)
    is_active = Column(Boolean, default=True)
    is_healthy = Column(Boolean, default=True)  # tracks consecutive-failure health

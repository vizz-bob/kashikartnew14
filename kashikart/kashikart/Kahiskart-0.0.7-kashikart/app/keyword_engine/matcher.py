...
class KeywordMatcher:
    ...
    async def match_tender(self, tender: Tender) -> List[Tuple[Keyword, str]]:
        ...
        document_text = "" # document_text not in model
    ...
    async def rebuild_all_matches(self) -> Dict[str, int]:
        """
        Rebuild keyword matches for all non-deleted tenders using the current
        active keyword set.
        """
        await self.load_keywords()

        await self.db.execute(delete(TenderKeywordMatch))
        await self.db.execute(
            update(Keyword).values(match_count=0, last_match_date=None)
        )

        result = await self.db.execute(
            select(Tender).where(Tender.is_deleted == False)
        )
        ...
        for tender in tenders:
            ...
            for keyword, location in matches:
                ...
                keyword.match_count += 1
                keyword.last_match_date = matched_at
                total_matches += 1

        await self.db.commit()

        logger.info(
            "Rebuilt keyword matches for %s tenders with %s total matches",
            matched_tenders,
            total_matches,
        )

        return {
            "processed_tenders": len(tenders),

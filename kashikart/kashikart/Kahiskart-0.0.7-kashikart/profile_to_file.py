...
async def run_profile():
    with open("final_profile_results.txt", "w") as f:
        async with AsyncSessionLocal() as db:
            ...
            f.write(f"Matched Recent Count: {res} | Time: {time.time()-s:.2f}s\n")
            ...
            rows = res.all()
            f.write(f"Top Keywords: {len(rows)} | Time: {time.time()-s:.2f}s\n")
